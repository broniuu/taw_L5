...
const dataEndpoint = (router) => {
    ...
    router.get('/api/post/:id', async (request, response, next) => {
       response.status(200).send({post: posts.find(x => x.id === Number(request.params.id))});
    });

    router.put('/api/post', async (request, response, next) => {
        let modifiedPost = request.body.modifiedPost;
        let foundIndex = posts.findIndex(x => x.id === modifiedPost.id);
        posts[foundIndex] = modifiedPost;
        response.status(200).send({post: posts[foundIndex]});
    });
};
